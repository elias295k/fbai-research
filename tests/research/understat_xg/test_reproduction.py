...
RESULT_PATH = Path(__file__).resolve().parents[3] / "research" / "understat_xg" / "result.json"
...
def test_historical_result_declares_source_design_and_four_roles() -> None:
    ...
    assert result["experiment_id"] == "understat_xg_lr68_historical_reproduction"
    assert result["status"] == "reproduced"
    assert result["class_order"] == ["H", "D", "A"]
    assert result["external_xg"]["feature_count"] == 16  # type: ignore[index]
...
def test_result_records_exact_failed_gate_and_disposition() -> None:
    result = _result()
    ...
    assert gate["observed"]["improving_fold_count"] == 2  # type: ignore[index]
...
def test_result_contains_no_paths_teams_rows_predictions_or_market_fields() -> None:
